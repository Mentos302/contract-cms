...
class ReportController extends Controller {
	...
	public function upcomingRenewalRevenue( Request $request ) {
		...
		if ( $startDate && $endDate ) {
			...
			foreach ( $renewableContracts as $contract ) {
				...
				$annualRevenue += ( $contract->contract_price - $contract->contract_cost ) * $contractTerm;
				$quarterlyRevenue += ( $contract->contract_price - $contract->contract_cost ) * $contractTerm * 4;
				$monthlyRevenue += ( $contract->contract_price - $contract->contract_cost ) * $contractTerm * 12;
			}
		}
	...
	public function lostContracts( Request $request ) {
		...
		if ( $startDate && $endDate ) {
			...
			$lostRevenue = $lostContracts->sum( function ($contract) {
				return $contract->contract_price - $contract->contract_cost;
			} );
		}
