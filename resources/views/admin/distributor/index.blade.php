...
@slot('title') Distributors @endslot
@endcomponent
<div class="row">
    <div class="col-xl-12">
        <div class="card">
            <div class="card-header d-md-flex justify-content-between">
                <div class="d-flex align-items-baseline">
                    <h4 class="card-title mb-0 flex-grow-1">Distributor</h4>
                    <a href="{{ route('distributor.create') }}" class="btn btn-outline-secondary mx-2">
                        <span class="icon-on"><i class="ri-add-line align-bottom me-1"></i> Add</span>
                    </a>
                </div>
                <form action="{{route('distributor.index')}}" method="GET" role="search">
                            ...
                            <tbody>
                                @foreach ($distributors as $key=> $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->name }}</td>
                                        <td class="d-flex">
                                            <a href="{{route('distributor.edit', $item->id)}}" class="btn btn-success btn-icon waves-effect waves-light"><i class=" ri-pencil-fill"></i></a>
